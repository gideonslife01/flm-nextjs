...
import type { ThemeName } from '../themeNames'; // ✅ myapp49
...
export function ThemeexTheme({ timeline, username, onBoost, onLike }: any) {
  const { theme, setTheme } = useTheme();

  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    //const newTheme = e.target.value;
     const newTheme = e.target.value as ThemeName;  // ✅ myapp49 : for type error
    console.log('🔄 select change ->', newTheme);

    // 1. Context 업데이트 / Contest Update
    setTheme(newTheme);

    // 2. localStorage 직접 저장 (혹시 Context를 사용하 수 없어도 보장)
    // Directly save to localStorage (ensures functionality even if Context cannot be used)
    try {
            ...
            <article key={`${p.source}-${p.id}`} className="pinafore-status">
              ...
              <div className="status-content">
                <div className="status-header">
                  ...
                  {/* ✅ myapp49 - isBoostedPost 또는 type=Announce면 부스트 라벨 */}
                  {(p.isBoostedPost || p.type === 'Announce' || p.source === 'inbox') && p.original_id && (
                    <span className="boost-label">🔁 {getDisplayName(p.actor)}boosted</span>
                  )}
