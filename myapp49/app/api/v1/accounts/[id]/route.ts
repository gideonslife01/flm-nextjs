// ✅ myapp41 - app/api/v1/accounts/[id]/route.ts 
// - Profile info.
  ...
  try {
    ...
    if (id.startsWith('remote_')) {
      ...
      try {
         ...
         **/
        const checkUser = me || 'user1'; // ✅ myapp41
        const followingCnt = db.prepare(`SELECT COUNT(*) as cnt FROM following WHERE username = ?`).get(`${checkUser}`) as any;
        const followersCnt = db.prepare(`SELECT COUNT(*) as cnt FROM followers WHERE username = ?`).get(`${checkUser}`) as any;
    ...
    // 로컬 / local - ✅ myapp41
    const targetUsername = id; // ✅ user1, user2...
    console.log(`✅ 로컬 프로필 조회 target=${targetUsername}`);

    // users 테이블에서 실제 유저 정보
    const userRow = db.prepare(`SELECT username, display_name, summary, created_at FROM users WHERE username =?`).get(targetUsername) as any;

    const cntPosts = db.prepare(`SELECT COUNT(*) as cnt FROM posts WHERE username = ?`).get(targetUsername) as any;
    const cntFollowing = db.prepare(`SELECT COUNT(*) as cnt FROM following WHERE username = ?`).get(targetUsername) as any;
    const cntFollowers = db.prepare(`SELECT COUNT(*) as cnt FROM followers WHERE username = ?`).get(targetUsername) as any;
