...
//  - ✅ myapp30 + myapp45 + myapp48 + myapp49
...
export async function GET(req: Request) {
  ...
  // Separated `viewer` and `target` (the previous code overwrote the username if a token was present, breaking profile lookups).
  
  // targetUsername = 프로필 주인, viewer = 지금 로그인한 사람
  // targetUsername = profile owner, viewer = currently logged-in user
  const viewer = getViewerFromRequest(req); // login user
  const profileOwner = targetUsername; // search user

  // ✅ myapp48 - target 체크 / Check the target.
  ...
  // ✅ myapp48 - visibility 필터링 추가 / Added visibility filtering.

  // public: 누구나 / unlisted: 로그인 필요 / private: 본인+팔로워만
  // public: Anyone / unlisted: Login required / private: You + followers only
