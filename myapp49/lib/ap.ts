...
async function signAndSend(inbox: string, doc: any, username: string) {
  ...
  const res = await fetch(inbox, {
    ...
    headers: {
      ...
      'Accept': 'application/activity+json',// ✅ myapp46 - add
      'Date': date,
      'Digest': digest,
      'Signature': sigHeader,
      //'Host': url.host // for mastodon ✅ myapp46 - comment processing
    },
    body
  });

  const text = await res.text();
  console.log(`📬 ${doc.type} ${res.status} -> ${inbox} | ${text.slice(0,200)}`); // ✅ add log myapp46
...
export async function sendNote(toInbox: string, note: any, username: string, postId: string, content: string) {
  ...
  const doc = {
    ...
    //cc: [`${actorId}/followers`],
     cc: [`https://${DOMAIN}/users/${username}/followers`], // ✅ myapp46
    object: note
  };
...
// ✅ myapp49 - Undo Like 보내기 / Send Undo Like
...
export async function sendDelete(toInbox: string, postId: string, username: string) {
  ...
  const objectId = postId.startsWith('http') ? postId : `${actorId}/posts/${postId}`;
