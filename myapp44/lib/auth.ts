...
export function getUsernameFromRequest(req: Request | NextRequest): string | null {
  try {
    ...
    // 2. Cookie
    const cookieHeader = (req as any).headers?.get?.('cookie') || req.headers.get('cookie') || '';
    ...
    if (refreshCookie) {
      try {
        const row = db.prepare(
          `SELECT username FROM sessions WHERE refresh_token =? AND expires_at > datetime('now')`
        ).get(refreshCookie) as any;
        if (row?.username) {
          console.log(`✅ me from refresh_token: ${row.username}`);
          return row.username;
        }
      } catch {}
    }
    ...
    console.error('getUsernameFromRequest error', e);
    return null;
  }
}
