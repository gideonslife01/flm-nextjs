// app/usersui/[username]/_components/themes/pinafore/PinaforeTheme.tsx - myapp26 fix ✅
...
export function MastodonTheme({ timeline, username, onBoost, onLike }: any) {
  ...
  return (
    <>
      <style>{`
       ...
       .pinafore-nav button,.pinafore-nav select {
          text-align: left;
          padding: 10px 12px;
      ...
      <div className="pinafore-layout">
        <nav className="pinafore-nav">
          <h2>🐘 {username} / Mastodon</h2>
          <button>🏠 Home</button>
          <button>🔔 Notifications</button>
          <button>👤 Profile</button>
         <select 
            value={theme} // <- 현재 로컬스토리지 값 표시
            onChange={(e) => {
            setTheme(e.target.value as any); // <- localStorage + state 둘 다 변경 (reload 필요없음)
         }}>
            ...
            <article key={`${p.source}-${p.id}`} className="pinafore-status">
              ...
              <div className="status-content">
                ...
                <div className="status-actions">
                  <button onClick={() => onBoost(p)} className={p.isMyBoost? 'boosted' : ''}>
                    🔁 {p.boostCount || ''}
                  </button>
