...
import { sendNote } from '@/lib/ap'; // <- 배달 함수!
...
export async function POST(req: Request) {
  ...
  // 1. ActivityPub Note 만들기
  const noteId = `https://yourhost.domain.org/users/${username}/posts/${id}`;
  const note = {
    id: noteId,
    type: 'Note',
    attributedTo: `https://yourhost.domain.org/users/${username}`,
    content: content,
    to: ['https://www.w3.org/ns/activitystreams#Public'],
    cc: [`https://yourhost.domain.org/users/${username}/followers`]
  };
